...
function LikeContainer({ likes, postId, isPublicAndPurchase, isOwnerPost, setToggleUpdatePost, postOwnerId }) {
    const { notiAddLike } = useContext(socketContext);
    ...
    const hdlClickAddLike = async postId => {
        ...
        if (myuser.id !== postOwnerId) {
            notiAddLike({
                postId,
                noticeType: "Like",
                userNoticeId: myuser?.id,
                interactedUserId: postOwnerId,
            });
        }
